...
| DTm-29 | BFS con `list.pop(0)` en `query_entities()` (`knowledge_graph.py`): O(n) por operacion, O(n²) total en traversal. Con grafos de 50K entidades es cuello de botella directo. Fix trivial: sustituir por `collections.deque` con `popleft()`. | Media | Abierto |
| DTm-30 | `query_by_keywords()` (`knowledge_graph.py`) sin indice: recorre toda `self._entities` y todas las aristas en cada llamada — O(entidades × keywords) por query. No existe indice invertido sobre nombres de entidad ni descripciones de relaciones. | Media | Abierto |
