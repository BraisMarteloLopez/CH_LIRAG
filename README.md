...
| DTm-38 | Fallback silencioso de estrategia: `LightRAGRetriever` degrada a SimpleVector si `HAS_NETWORKX=False` o `llm_service` es None, pero `config_snapshot` y `run_id` siguen reportando `LIGHT_RAG`. El evaluator no valida que la estrategia ejecutada coincida con la configurada. Resultado: runs etiquetados como LIGHT_RAG que ejecutan SimpleVector+Reranker, invalidando conclusiones comparativas. Fix: registrar `strategy_actual` en resultados y/o abortar si la estrategia real no coincide con la configurada. | Alta | Abierto |
