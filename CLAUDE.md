...
| 16 | **Synthesis timeouts en corpus pequeno** | **MEDIO (observacional)** | Smoke-test `mteb_hotpotqa_20260418_183018` (800 docs, 25q, hybrid, `KG_SYNTHESIS_TIMEOUT_S=90`, `KG_SYNTHESIS_MAX_CHARS=0`) reporto `kg_synthesis_stats.fallback_rate=0.24` (6/25 timeouts, 0 errores, 0 empty); run posterior `mteb_hotpotqa_20260418_223530` (1000 docs, 35q, hybrid) reporto `fallback_rate=0.57` (20/35 timeouts) — la degradacion **empeoro** con el corpus mas grande. El umbral de CLAUDE.md es 10% — ningun run refleja la arquitectura completa. En un corpus de 800-1000 docs no deberia haber saturacion de entidades/relaciones; las hipotesis a discriminar son: (a) prompt de synthesis demasiado largo cuando `KG_SYNTHESIS_MAX_CHARS=0` deja pasar contexto de 192K chars, (b) latencia del LLM bajo carga concurrente, (c) timeout de 90s insuficiente para el NIM configurado. Sin resolver, cualquier run LIGHT_RAG con synthesis ON es inconclusivo porque una fraccion no despreciable de queries se evalua contra el contexto estructurado — no contra la narrativa que es la tesis del paper | **Instrumentacion landed** (Fase C del HANDOFF): `kg_synthesis_stats` ahora expone `p50/p95/max_total_ms`, `p50/p95/max_queue_ms`, `p50/p95/max_llm_ms` via hook `timing_out` en `AsyncLLMService.invoke_async`. El proximo run diagnostico podra discriminar (a)(b)(c) segun la interpretacion de estos percentiles — ver "Observabilidad de runs" y HANDOFF paso 3 para el arbol de decision. Mientras tanto, ningun run con `fallback_rate > 0.1` vale como evidencia de P0 — re-ejecutar o ajustar `KG_SYNTHESIS_TIMEOUT_S` / `KG_SYNTHESIS_MAX_CHARS` / `NIM_MAX_CONCURRENT_REQUESTS` |
