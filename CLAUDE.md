...
- **#7 / #12 — Formato de contexto: JSON-lines con reference_id en vez de CSV con headers:** Permite que la capa de synthesis ancle citas [ref:N] al reference_id de cada chunk. Cambiar a CSV romperia el esquema de citas sin beneficio. Instrumentacion de citas (citation_refs_{synth,gen}_*, hallazgo gen_*=0, acoplamiento al prompt) documentada en "Observabilidad de runs".
...
| 11 | Duplicacion parcial de iteracion de vecinos en KG | **BAJO** | `_get_neighbors_weighted()` y `get_neighbors_ranked()` (div #9) iteran los mismos edges con campos distintos. Duplicacion deliberada para no complicar la API interna | Refactorizar a iterador comun solo si aparece un tercer consumidor |
