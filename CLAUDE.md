...
Estado a fecha: **todas las divergencias arquitectonicas (#2, #4+5, #6, #7, #8, #9, #10) estan resueltas en codigo y con tests unitarios**. De las 7: **3 tienen validacion empirica directa de correctitud** en el run `032905` (#2, #6, #8) via observables que discriminaron calidad del comportamiento; **3 tienen observable per-query + validacion empirica complementaria** tras el run `181645` — `kg_budget_cap_triggered=False` en 35/35 (#4+5, esperado con ventana 192K), `kg_entities_with_neighbors=18.2 mean, mean_neighbors=3.48` (#9, canal operando en 100%), `citation_refs_synth_*` validado (62 validas, 0 out_of_range, 0 malformed, coverage_ratio 0.927) con hallazgo abierto en `citation_refs_gen_*=0` (generador no propaga `[ref:N]` al usuario final; ver fila de divergencia #7 para detalle); **1 tiene validacion de presencia pero no de calidad** (#10 — piggyback puede degradar el output high-level vs la llamada dedicada del paper; el observable actual solo mide matches de la VDB, no calidad semantica de las keywords). Ver "Status de validacion por divergencia arquitectonica" y la fila de divergencia #10 para el desglose. La validacion de ejecucion estable (Pre-P0 criterio 2) cerro con el run `032905` (fallback_rate 2.86% < 10% umbral) y fue reproducida por el run `181645` (2.857%). Las funcionalidades extra propias del entorno (cache de KG, fallbacks ante errores, instrumentacion de timing) son adaptaciones operativas documentadas, no sustitutos de piezas del paper. **El riesgo de piggyback (#10) es bloqueante antes de P2** (escenario catalogo especializado) pero no de P0 (HotpotQA es ciego a este tipo de degradacion por saturacion del retrieval vector directo).
