...
|---|---|---|---|
| `KnowledgeGraph.query_by_entities(entity_names, max_hops, max_docs, pre_resolved)` | `knowledge_graph.py:613` | BFS ponderado desde entidades query → ranking de `doc_ids` con scoring por hops + edge weight (DTm-72) + confidence. Es el equivalente a `_find_most_related_text_unit_from_entities` del paper | Definida, con tests (KG*), **no llamada desde `shared/` ni `sandbox_mteb/`** |
...
2. **No anadas uso de estas funciones sin discutir la divergencia #8 primero**. Conectar `query_by_entities` al ranking cambia el significado de las metricas de retrieval del proyecto — es una decision estrategica, no una mejora mecanica.
