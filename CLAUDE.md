...
|---|---|---|---|
| 4 | **Contexto aplanado a ranking de docs** — el original presenta entidades, relaciones y chunks como **secciones separadas** en el prompt del LLM; aqui todo se aplana a un ranking unico de doc_ids | **9/10** | El LLM del original recibe contexto estructurado (`"Knowledge Graph Data:"` + `"Document Chunks:"`) con presupuesto de tokens por tipo. El codigo para generarlo existe (`format_structured_context()` en `retrieval_executor.py:195-266`, invocado desde `generation_executor.py:89-103`), pero solo se activa cuando `retrieval_metadata` contiene `kg_entities`/`kg_relations`. En `_fuse_with_graph()` (usado por `hybrid`, `local`, `global`) estos campos no se propagan — el contexto estructurado nunca se genera para los modos del paper. Ademas, `graph_primary` es un modo propio de CH_LIRAG (DAM-3) que no existe en el paper original y debe eliminarse. **Modos validos (paper)**: `naive`, `local`, `global`, `hybrid` — todos deben presentar secciones separadas al LLM. Fix: (1) propagar `kg_entities`/`kg_relations` desde `_fuse_with_graph()`, (2) eliminar `graph_primary`. |
