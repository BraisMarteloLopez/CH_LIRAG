...
Estado a fecha: **todas las divergencias arquitectonicas (#2, #4+5, #6, #7, #8, #9, #10) estan resueltas en codigo y con tests unitarios**. De las 7: **3 tienen validacion empirica directa de correctitud** en el run `032905` (#2, #6, #8) via observables que discriminaron calidad del comportamiento; **2 tienen observable per-query disponible para runs post-auditoria pero aun sin datos del run empirico** (#4+5 → `kg_budget_cap_triggered`, #9 → `kg_entities_with_neighbors`/`kg_mean_neighbors_per_entity`); **1 sigue ejercitada implicitamente sin observable dedicado** (#7 — formato JSON-lines con `reference_id`); **1 tiene validacion de presencia pero no de calidad** (#10 — piggyback puede degradar el output high-level vs la llamada dedicada del paper; el observable actual solo mide matches de la VDB, no calidad semantica de las keywords). Ver "Status de validacion por divergencia arquitectonica" y la fila de divergencia #10 para el desglose. La validacion de ejecucion estable (Pre-P0 criterio 2) cerro con el run `032905` (fallback_rate 2.86% < 10% umbral). Las funcionalidades extra propias del entorno (cache de KG, fallbacks ante errores, instrumentacion de timing) son adaptaciones operativas documentadas, no sustitutos de piezas del paper. P0 sobre un benchmark de contra-referencia validara implicitamente #7 si el delta reproduce la direccion del paper; #4+5 y #9 dispondran de dato observable directo desde el primer run post-auditoria. **El riesgo de piggyback (#10) es bloqueante antes de P2** (escenario catalogo especializado) pero no de P0 (HotpotQA es ciego a este tipo de degradacion por saturacion del retrieval vector directo).
...
| 15 | **`retrieval_metadata` por query — exportacion JSON/CSV** | **RESUELTA** | Fix unificado sobre ambos exports. (1) **JSON**: `QueryEvaluationResult.to_dict()` en `shared/types.py` ahora emite un bloque `retrieval_metadata` per-query con el subset relevante — `lightrag_mode`, `kg_fallback` (`no_keywords`/`no_doc_ids`/`docs_not_in_store`/ausente), `kg_chunk_keyword_matches`, `kg_entities_count`, `kg_relations_count`, `kg_synthesis_used`, `kg_synthesis_error` (`timeout`/`error`/`empty`/ausente), `graph_active`, y — anadidos en la auditoria de observabilidad — `kg_entities_with_neighbors`, `kg_mean_neighbors_per_entity` (divergencia #9) y `kg_budget_cap_triggered` (divergencia #4+5). Listas completas de entidades/relaciones se colapsan a conteos para no inflar el JSON; queries SIMPLE_VECTOR no emiten el bloque (omitted si subset vacio). (2) **CSV**: `shared/report.py:to_detail_csv()` tenia un guard muerto que buscaba `graph_candidates` (clave eliminada con la resolucion de divergencia #8), por lo que el bloque `if has_lightrag:` nunca ejecutaba y el detail.csv no tenia columnas KG en runs reales (auditado en `mteb_hotpotqa_20260418_223530_detail.csv`, cabecera sin `graph_*`). El guard y las columnas se reescribieron usando `extract_retrieval_metadata_subset()` como unica fuente de verdad compartida con el JSON: columnas `lightrag_mode`, `kg_fallback`, `kg_entities_count`, `kg_relations_count`, `kg_chunk_keyword_matches`, `kg_entities_with_neighbors`, `kg_mean_neighbors_per_entity`, `kg_budget_cap_triggered`, `kg_synthesis_used`, `kg_synthesis_error`. (3) **Synthesis outcome per-query**: `_synthesize_kg_context_async` ahora retorna `Tuple[str, Optional[str]]` con el `error_code`, y `_process_single_async` escribe `kg_synthesis_used`/`kg_synthesis_error` en `retrieval_detail.retrieval_metadata` para que ambos exports lo consuman. Tests: `test_report.py` (4 casos iniciales JSON + CSV + regresion guard SIMPLE_VECTOR; +2 casos nuevos para los observables de #9 y #4+5), `test_kg_synthesis.py::TestPerQuerySynthesisMetadata` (5 casos cubriendo success/timeout/empty/error/no-kg-data). Listo para usar en el run diagnostico del paso 2 del HANDOFF. | — |
